...
const sendNotificaton = (token, title, body) => {
  ...
  let config = {
    ...
    headers: {
      ...
      Authorization:
        "Bearer AAAAr7Oo3UE:APA91bFFVitm9Wv5Gaa386gzdGwiKc3yfnU0o6BC1GSRrqG2wcF8HP9EqEaN_HaWdN2aVvo7_Jl14X15rSXrFCO2p6-_MJXLPqei2E-HRZhDhy2OTrD3P9oWtujFtu8Lm4mvdBUi8TWV",
    },
    data: message,
  };
...
const adminNotification = async (req, res) => {
  ...
  try {
    ...
    console.log(
      "🚀 ~ file: firebaseController.js:78 ~ adminNotification ~ users:",
      users
    );
    users
      .map(
        (val) => !["manufacturer", "admin"].includes(val.role) && val.fbToken
      )
      .filter(Boolean)
      .map((token) => {
        console.log(
          "🚀 ~ file: firebaseController.js:81 ~ users.map ~ token:",
          token
        );
        sendNotificaton(token, title, description);
      });
    res.status(200).json({ msg: "Notification sent successfully" });
  } catch (error) {
    console.log(error);
